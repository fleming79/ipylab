...
async def test_on_ready_add_and_remove():
    obj = Ipylab()
    callback = MagicMock()

    # Add the callback
    obj.on_ready(callback)
    assert callback in obj._on_ready_callbacks  # noqa: SLF001
    ...
    assert callback not in obj._on_ready_callbacks  # noqa: SLF001
...
async def test_on_ready_async(app: App):  # noqa: ARG001
    obj = Ipylab()
    callback = AsyncMock()
