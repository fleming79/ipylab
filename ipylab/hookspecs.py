...
async def autostart_once(app: ipylab.JupyterFrontEnd) -> None | Awaitable[None]:
...
@hookspec(firstresult=True)
async def vpath_getter(app: ipylab.JupyterFrontEnd, kwgs: dict) -> str:  # pyright: ignore[reportReturnType]
...
@hookspec(firstresult=True)
def get_logging_handler(app: ipylab.JupyterFrontEnd) -> IpylabLogHandler:  # pyright: ignore[reportReturnType]
